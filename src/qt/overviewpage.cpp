...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::voi)
    {
    }
...
void OverviewPage::setBalance(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance, const CAmount& anonymizedBalance, const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    ui->label_voi4->setVisible(showImmature || showWatchOnlyImmature);
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    if (model && model->getOptionsModel()) {
        ...
        connect(ui->blabel_voi, SIGNAL(clicked()), this, SLOT(openMyAddresses()));

    }

    // update the display unit, to not use the default ("voi")
    updateDisplayUnit();
}
...
double BlockRewardvoi =  static_cast<double>(BlockReward/COIN);
//int64_t voiSupply = chainActive.Tip()->nMoneySupply / COIN;
...
ui->label_CurrentBlockReward_value->setText(QString::number(BlockRewardvoi));
//ui->label_voiSupply_value->setText(QString::number(voiSupply));
...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizevoiAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizevoiAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizevoiAmount = strAnonymizevoiAmount.remove(strAnonymizevoiAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizevoiAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizevoiAmount * COIN) nMaxToAnonymize = nAnonymizevoiAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizevoiAmount * COIN) {
        // ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
        //                                       .arg(strAnonymizevoiAmount));
        strAnonymizevoiAmount = strAnonymizevoiAmount.remove(strAnonymizevoiAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizevoiAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
